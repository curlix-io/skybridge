...
func TestCertVerificationErrorHintDetectsX509Errors(t *testing.T) {
	...
	if !strings.Contains(hint, "SKYBRIDGE_STUDIO_GATEWAY") || !strings.Contains(hint, "SKYBRIDGE_CA_BUNDLE_PEM") {
...
func TestLogCertHintOnceLogsOnlyOnce(t *testing.T) {
	...
	c := New(Config{Target: "127.0.0.1:0"}, slog.New(slog.NewTextHandler(&buf, nil)))
