...
func TestServeK8sStreamMissingClientTLS(t *testing.T) {
	...
	serveK8sStream(context.Background(), st, nil, tunnel.OpenMeta{Target: "cluster", Addr: "x:6443"}, config.Agent{}, slog.New(slog.NewTextHandler(&buf, nil)))
...
func TestServeK8sStreamProxyInjectFailsFastOnClosedClient(t *testing.T) {
	...
	serveK8sStream(context.Background(), st, nil, tunnel.OpenMeta{Target: "cluster", Addr: upstreamLn.Addr().String()}, cfg, slog.New(slog.NewTextHandler(&buf, nil)))
	serverSess.Close()
...
func TestServeK8sStreamDialFailure(t *testing.T) {
	...
	serveK8sStream(ctx, st, nil, tunnel.OpenMeta{Target: "cluster", Addr: "127.0.0.1:1"}, cfg, slog.New(slog.NewTextHandler(&buf, nil)))
