...
func serveK8sStream(ctx context.Context, st *tunnel.Stream, meta tunnel.OpenMeta, cfg config.Agent, logger *slog.Logger) {
	...
	if len(cfg.K8sClientTLSCertPEM) == 0 || len(cfg.K8sClientTLSKeyPEM) == 0 {
		logger.Error(fmt.Sprintf("stream open: target %q is a kubernetes proxy but SKYBRIDGE_K8S_CLIENT_TLS_CERT_PEM/_KEY_PEM is not set", meta.Target))
		return
	}
	...
	if err != nil {
		logger.Error(fmt.Sprintf("stream open: kubernetes proxy client TLS cert/key: %v", err))
		return
	}
